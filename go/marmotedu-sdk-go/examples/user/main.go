...
func main() {
	...
	config, err := clientcmd.BuildConfigFromFlags("", *iamconfig)//根据ymal文件拿到config
	if err != nil {
		panic(err.Error())
	}

	// create the iamclient
	iamclient, err := iam.NewForConfig(config)//根据config创建一个restclient
	if err != nil {
		panic(err.Error())
	}

	usersClient := iamclient.APIV1().Users()//得到restclient
